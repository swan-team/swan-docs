...
开发者需要为当前的小程序页面配置 seo 相关的信息，包括 title、description、keywords，这三类信息需要保持和小程序对应的 H5 站页面一致。具体接口详见<a href="http://smartapp.baidu.com/docs/develop/api/seo/#setDocumentTitle/">配置 seo</a>。

由于每个页面的 seo 信息和页面内容强相关，建议先通过 swan.request 请求开发者  server ，由开发者 server 返回 seo 信息，再通过 <a href="https://smartprogram.baidu.com/docs/develop/api/seo/">seo 接口</a> 设置到页面中。
...
* 也可以在“流量配置-<a href="https://smartprogram.baidu.com/developer/home/promotion.html?appId=11969750&tabCur=search">自然搜索结果</a>”中完成熊掌号的在线注册绑定。

## 配置 H5 域名

> 已获取自然搜索结果的配置资格。

1. 进入智能小程序平台，单击进入<a href="https://smartprogram.baidu.com/developer/index.html">小程序首页</a>，单击“流量配置>自然搜索结果”。
...
* 开发者 H5 站点的域名，应先在“业务域名”中进行添加。
* 如果开发者存在多个 H5 子域，且这些 H5子 域都有对应的小程序页面，请配置 H5 主域名。
    举例，如果存在多个 H5 子域：
    https://club.m.autohome.com.cn
    https://k.m.autohome.com.cn
    https://chejiahao.m.autohome.com.cn
    请在小程序平台配置主域名：https://m.auto.home.com.cn
* 对于新站小程序可不填 H5 域名，此时自然搜索的内容为新站点内容的收录。

## 配置url映射规则

url映射规则定义的是小程序页面到 H5 页面的映射规则，百度搜索基于这一份映射规则来计算小程序的排序权重以及和 H5 页面的去重处理等等，因此配置 url 映射规则对小程序入搜索至关重要。
### 增加 url-mapping 字段
...
**字段**： Object
**说明**：
* 在pages 中配置的小程序路径作为 key，每一个小程序路径对应的 H5 页面作为 value; 每一对”key-value”作为一个完整的 url 映射。
...
代码包通过审核后，开发者将会自动获得一个Web小程序的线上地址，请参见<a href="https://smartprogram.baidu.com/docs/develop/web/detail/">代理域名配置详解</a>。
...
**sitemap 示例**:
```
pages/room?roomId=9999
pages/room?roomId=28801
pages/room?roomId=42455
pages/room?roomId=91
pages/room?roomId=68
pages/room?roomId=488062
pages/room?roomId=67332
pages/room?roomId=66481
pages/room?roomId=186376
pages/room?roomId=7101
pages/room?roomId=9629
pages/room?roomId=22934
pages/room?roomId=98
pages/room?roomId=999
pages/room?roomId=169112
pages/room?roomId=24143
```
目前支持存量上传和增量上传两种方式:
