...
<a href="swanide://fragment/28fe81e0cea9ae54f9651113d6bc8c911576055322440" title="在开发者工具中预览效果" target="_self">在开发者工具中预览效果</a>
...
Page({
    ...
    setStorageSync() {
        ...
        // 基础库 3.130.1 之前，通过 instanceof 来判断是否未传必传值 key 和 data，通过返回的原生对象中是否有 errCode 和 errMsg 来判断接口是否调用失败
        // 基础库 3.130.1 及以后，通过 instanceof 来判断是否未传必传值 key 和 data 或接口是否调用失败
...
* 基础库 3.130.1 之前，未传必传值`key`和`data`时，会返回一个标准的`Error`对象，可通过`instanceof`来判断是否未传必传值`key`；接口调用失败时会返回一个包含`errCode`和`errMsg`的原生对象，详见上述错误码。
* 基础库 3.130.1 及以后，未传必传值`key`和`data`或接口调用失败时都会返回一个标准的`Error`对象，可通过`instanceof`来判断接口是否调用失败。
