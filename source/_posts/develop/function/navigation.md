...
在智能小程序中，导航跳转是连接两个页面的纽带，由于框架导航实现底层原理的黑盒效应，以及页面容器的场景和类型不同，导致我们在开发小程序过程中，不能清晰的判断页面的一些表现（例如不清晰什么场景会出现左上角的返回按钮和回首页的小房子图标），通过对SWAN框架进行分析，我们整理了全面的导航原理以及页面跳转后客户端的表现，希望也能使得其他开发者更加的清晰。

# 一、名词解释

- 左上角返回按钮：客户端绘制的一个左箭头图标，位于顶部导航栏的最左侧，用于返回上一个小程序页面容器；
- 小房子图标：客户端绘制的一个小房子图标，用于打开小程序app.json配置的首页
- 正常小程序页面：非使用web-view组件承载h5的页面
- 二级页面：非app.json pages配置的首项页面或者非tabBar页面

# 二、页面跳转底层原理

智能小程序提供了5种导航方式（[路由方式传送门](https://smartprogram.baidu.com/docs/develop/framework/app_service_getcurrentpages/#%E8%B7%AF%E7%94%B1%E6%96%B9%E5%BC%8F/)），即: navigateTo、redirectTo、navigateBack、switchTab、reLaunch，除直接调用API的方式，还可以通过navigator组件绑定属性的方式触发路由跳转（[navigator组件传送门](https://smartprogram.baidu.com/docs/develop/component/nav/#navigator/)）。
...
- 当首次打开的页面为非二级页面，那么此页页面不会有返回按钮和小房子图标，此时如果通过跳转下一个新的页面（客户端新开一个容器，且不为tabBar页面），则会在顶部导航栏左侧增加一个返回按钮；
- 当首次打开的页面为二级页面，那么此页面就会出现小房子图标，此时如果通过导航跳转下一个新的页面（客户端新开一个容器，且不为tabBar页面），则会在小房子图标左侧增加一个返回按钮；
...
TIPS：在web-view组件承载h5页面中，如果想要跳转到一个新的小程序页面（即客户端新开一个容器），需要配合jssdk进行操作（[web-view组件传送门](https://smartprogram.baidu.com/docs/develop/component/open/#web-view/)）。

### 2.场景举例

最近有业务同学想接入小程序，为了从h5无缝转接到小程序，首先想到的就是小程序内放置一个`web-view`组件来链接之前的h5页面，一行代码就完成了迁移到小程序的难题，真的是太开心了！
但是实际情况是怎么样的呢，看一下效果
![图片](http://agroup-bos.cdn.bcebos.com/69bc357409516f38e0dd385c39efcb3609a7c647)
...
- 解决办法 1： 
客户端上可以通过侧滑可以返回到webview组件的上一个个网页内容。这个办法缺点是一方面没有原生导航栈返回的动画效果，另一个方面作为用户来说，这个操作隐藏的比较深，难以发现。`不推荐使用`

- 解决办法 2：
有些同学可以会想到，是否可以在每个H5页面都加上一个带返回按钮的导航栏？
这种方案不可取，因为小程序页面的头部已经有个导航栏了，再加一个的话就显得非常的不协调，有点丑。`不推荐使用`

- 解决办法 3：
有熟悉的同学可以会想到自定义导航栏样式,自己搞一个导航栏,在app.json或者对应page.json增加配置（`navigationStyle: custom`）（[app配置传送门](https://smartprogram.baidu.com/docs/develop/tutorial/process/#%E9%85%8D%E7%BD%AE-app-js-%E6%96%87%E4%BB%B6/)）
NO,NO,这个也是行不通的，webview组件页面不支持自定义导航栏样式![图片](http://agroup-bos.cdn.bcebos.com/f08568f622c725483f9ff1797891b574dd3ab52b)`无法解决问题`


看了半天，到底能不能解决啊？当然有，下面是来说最后一种完美解决方案

- 解决办法 4 `(强烈推荐)`：
通过跳转新的小程序页面，比如在将要要加载的h5也放个webview组件，触发新的小程序页面加载，下一个页面的左上角是会有返回按钮，体验上和原生一致。

效果图:
![图片](http://agroup-bos.cdn.bcebos.com/18184a57557866fff35d42ccd97f8341f224721c)
...
举个栗子：
![图片](http://agroup-bos.cdn.bcebos.com/5046db5bcc77d73b7c04105cb99f229517130f6a)

`小程序页面栈内部导航流程图：`
![图片](http://agroup-bos.cdn.bcebos.com/bdaeccad859eb455db171607dda7906efec316a3)
...
`小程序间的导航流程图：`
![图片](http://agroup-bos.cdn.bcebos.com/680eecb07fb38f0d3d1584b63ba00d2254408f3d)

# 六.参考

[顶部导航栏的UI设计参考文档](https://smartprogram.baidu.com/docs/design/component/topnav/)

[导航路由开发文档](https://smartprogram.baidu.com/docs/develop/api/show_tab/)

[百度webview组件开发文档](https://smartprogram.baidu.com/docs/develop/component/open/#web-view/)