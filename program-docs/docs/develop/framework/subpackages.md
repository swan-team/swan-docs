...
在小程序启动时，会下载主包和所有分包（包括独立分包）用到的资源。
...
在 Web 态中独立分包与普通分包一样，会在小程序启动时下载用到的资源。
...
Web 态的分包所需的资源会在访问小程序时加载，由于这与端内小程序的加载逻辑不同，因此在独立分包中可以通过`getApp`获得真正的`App`对象，同时也可以使用主包中`app.css`的样式。
**请开发者以端内小程序的表现为基准**，我们会在 Web 态的后续版本中修复上述问题。
